...
test('invalid input to email shows error', async () => {
  const { getByLabelText, getByText, getByRole } = renderWithRouter(<SignupForm />)

  await fireEvent.change(getByLabelText('User Email'), { target: { value: 'example@love' }})
...
test('invalid input to password shows error', async () => {
  ...
  await fireEvent.change(getByLabelText('Confirm Password'), { target: { value: 'passwprd' }})
  await fireEvent.click(getByRole('button'))

  expect(getByText('Passwords dont match. Type in the same password')).toBeInTheDocument()
})
